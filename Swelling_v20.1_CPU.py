"""
Swelling_v20.1_CPU.py

CPU (NumPy) fork of the GPU swelling solver: CuPy is aliased to NumPy and
the raw CUDA force kernel is replaced by a vectorized NumPy port
(force_kernel_cpu). A finite solvent reservoir feeds a conservative FEM
...
4. Precision
       All arrays and the force kernel run in float64 on the CPU.

Output
------
    results/swelling_v20_cpu.h5      per-frame geometry and fields
    results/swelling_v20_cpu.xdmf    ParaView time-series wrapper
"""

import numpy as np
cp = np                                  # was: import cupy as cp
from scipy import sparse as sp_cpu
cpsp = sp_cpu                            # was: import cupyx.scipy.sparse as cpsp
import h5py
...
k_phi            = 1.0      # mu_chem -> phi conversion factor
...
h5_filename   = 'results/swelling_v20_cpu.h5'
xdmf_filename = 'results/swelling_v20_cpu.xdmf'
...
# ---------------------------------------------------------------------------
# Vectorized NumPy force kernel (Arruda-Boyce, prism FEM)
# ---------------------------------------------------------------------------

#   CPU port of the CUDA force_kernel below: identical math, vectorized over
#   elements and looping only the 8 Gauss points. Degenerate (|dJ|<dJ_min) and
#   inverted (Je<=0) elements are masked out per Gauss point instead of 'continue'.
# Gauss points/weights - same scheme as the CUDA kernel and the FEM Laplacian
_GX = np.array([1/3, 1/6, 1/6, 1/3, 1/3, 1/6, 1/6, 1/3])
_GY = np.array([1/3, 1/6, 1/3, 1/6, 1/3, 1/6, 1/3, 1/6])
_GZ = np.array([-1, -1, -1, -1, 1, 1, 1, 1]) * 0.5773502691896257
_GW = np.array([1/3, 1/6, 1/6, 1/6, 1/3, 1/6, 1/6, 1/6])

def force_kernel_cpu(def_pos_flat, ref_pos_flat, cells, mu_0, N_val, lam_0, sf,
                     phi_nodes, f_out, n_elem, N_x, N_y, dJ_min):
    """
    CPU/NumPy equivalent of the CUDA force_kernel.

    Identical math to the reference CUDA kernel, vectorized over elements and
    looping only the 8 Gauss points. Degenerate (|dJ| < dJ_min) and inverted
    (Je <= 0) elements are masked out per Gauss point instead of skipped, and
    the per-node force contributions are scattered into f_out in place.

    Parameters
    ----------
    def_pos_flat, ref_pos_flat : ndarray, shape (n_nodes*3,)
        Deformed and reference nodal positions, row-major (x, y, z per node).
    cells : ndarray, shape (n_elem*6,)
        Flattened prism connectivity (6 node indices per element).
    mu_0, lam_0 : float
        Shear and bulk (Lame) moduli [Pa].
    N_val, N_x, N_y : float
        Arruda-Boyce chain segments along z, x, y.
    sf : float
        phi-dependent softening factor for mu_0 / lam_0.
    phi_nodes : ndarray, shape (n_nodes,)
        Nodal solvent volume fraction.
    f_out : ndarray, shape (n_nodes*3,)
        Output force vector, written in place (not zeroed here).
    n_elem : int
        Number of prism elements.
    dJ_min : float
        Mesh-relative degenerate / div-by-zero guard on the Jacobian.
    """
    invNx, invNy, invNz = 1.0 / N_x, 1.0 / N_y, 1.0 / N_val

    conn   = cells.reshape(n_elem, 6)                      # (ne, 6)
    ref_e  = ref_pos_flat.reshape(-1, 3)[conn]             # (ne, 6, 3)
    def_e  = def_pos_flat.reshape(-1, 3)[conn]             # (ne, 6, 3)
    phi_e  = phi_nodes[conn]                               # (ne, 6)

    ef = np.zeros((n_elem, 6, 3), dtype=np.float64)
    eye = np.eye(3)

    for g in range(8):
        xi, eta, zeta, w = _GX[g], _GY[g], _GZ[g], _GW[g]
        lam3 = 1.0 - xi - eta
        zm   = 0.5 * (1.0 - zeta)
        zp   = 0.5 * (1.0 + zeta)
        Ns   = np.array([lam3*zm, xi*zm, eta*zm, lam3*zp, xi*zp, eta*zp])   # (6,)

        phi_gp = phi_e @ Ns                                # (ne,)
        phi_c  = np.minimum(phi_gp, 1.0)
        mu_loc = mu_0 * (1.0 - sf * phi_c)
        lm_loc = lam_0 * (1.0 - sf * phi_c)
        mu_h   = 0.5 * mu_loc
        J_sw   = 1.0 + phi_gp
        Jisq   = 1.0 / np.sqrt(J_sw)

        dNxi = np.array([
            [-zm,        zm,       0.0,      -zp,        zp,       0.0     ],
            [-zm,        0.0,      zm,       -zp,        0.0,      zp      ],
            [-0.5*lam3, -0.5*xi,  -0.5*eta,   0.5*lam3,  0.5*xi,   0.5*eta ],
        ])                                                 # (3, 6)

        J   = np.einsum('ik,ekj->eij', dNxi, ref_e)        # (ne, 3, 3)
        dJ  = np.linalg.det(J)                             # (ne,)
        ok1 = np.abs(dJ) >= dJ_min                         # degenerate/div-by-zero guard
        Js  = np.where(ok1[:, None, None], J, eye)         # avoid singular inv
        Ji  = np.linalg.inv(Js)                            # (ne, 3, 3)
        dNx = np.einsum('eik,kj->eij', Ji, dNxi)           # (ne, 3, 6) spatial grads

        # Fm[e,i,j] = sum_k def_e[e,k,i] * dNx[e,j,k]
        Fm  = np.einsum('eki,ejk->eij', def_e, dNx)        # (ne, 3, 3)
        Fe  = Fm * Jisq[:, None, None]

        Je  = np.linalg.det(Fe)                            # (ne,)
        ok  = ok1 & (Je > 0.0)
        Fes = np.where(ok[:, None, None], Fe, eye)         # guard inv / log
        Jes = np.where(ok, Je, 1.0)

        # Directional invariants: I4 = ||column of Fe||^2  -> (ne, 3)
        I4  = np.einsum('eij,eij->ej', Fes, Fes)
        I4x, I4y, I4z = I4[:, 0], I4[:, 1], I4[:, 2]
        dWx = mu_h/3.0 + 0.1*mu_loc*invNx*I4x + (11.0/350.0)*mu_loc*invNx*invNx*I4x*I4x
        dWy = mu_h/3.0 + 0.1*mu_loc*invNy*I4y + (11.0/350.0)*mu_loc*invNy*invNy*I4y*I4y
        dWz = mu_h/3.0 + 0.1*mu_loc*invNz*I4z + (11.0/350.0)*mu_loc*invNz*invNz*I4z*I4z

        cvol = lm_loc * np.log(Jes)
        Fei  = np.linalg.inv(Fes)                          # (ne, 3, 3)

        sq  = np.sqrt(J_sw)
        cv2 = cvol * sq
        P   = np.empty((n_elem, 3, 3), dtype=np.float64)
        # P[e,i,c] = 2*sq*dW_c*Fe[e,i,c] + cv2*Fei[e,c,i]
        P[:, :, 0] = 2.0*sq[:, None]*dWx[:, None]*Fes[:, :, 0] + cv2[:, None]*Fei[:, 0, :]
        P[:, :, 1] = 2.0*sq[:, None]*dWy[:, None]*Fes[:, :, 1] + cv2[:, None]*Fei[:, 1, :]
        P[:, :, 2] = 2.0*sq[:, None]*dWz[:, None]*Fes[:, :, 2] + cv2[:, None]*Fei[:, 2, :]

        dV = dJ * w
        # ef[e,j,i] -= dV * sum_k P[e,i,k] * dNx[e,k,j]
        contrib = np.einsum('eik,ekj->eji', P, dNx)        # (ne, 6, 3)
        ef += np.where(ok[:, None, None], -dV[:, None, None] * contrib, 0.0)

    # Scatter element node forces into the global (flat) force vector
    f_nodes = f_out.reshape(-1, 3)
    np.add.at(f_nodes, conn, ef)

# ---------------------------------------------------------------------------
# Original CUDA C force kernel (reference only, not compiled on CPU)
# ---------------------------------------------------------------------------

_FORCE_KERNEL_CODE_REFERENCE = r"""
...
# (end of reference CUDA kernel string; see force_kernel_cpu above)

# ---------------------------------------------------------------------------
# Allocate CPU work arrays (float64; no device transfer needed)
# ---------------------------------------------------------------------------

print("Allocating CPU arrays (float64)...", flush=True)
points_ref_gpu  = np.asarray(points_ref, dtype=np.float64)         # (n_nodes, 3)
ref_pos_flat    = np.asarray(points_ref.ravel(), dtype=np.float64) # (n_nodes*3,)
prism_cells_gpu = np.asarray(prism_cells.ravel())                 # (n_elem*6,)  int32
mu_chem_gpu     = np.asarray(mu_chem, dtype=np.float64)           # (n_nodes,)
phi_gpu         = np.asarray(phi, dtype=np.float64)               # (n_nodes,)
u_gpu           = np.zeros((n_nodes, 3), dtype=np.float64)
v_gpu           = np.zeros((n_nodes, 3), dtype=np.float64)
f_out_gpu       = np.zeros(n_nodes * 3, dtype=np.float64)
reservoir_gpu   = np.asarray(reservoir_mask)

# Conservative diffusion operators (float64)
K_gpu        = cpsp.csr_matrix(K_cpu)
node_vol_gpu = np.asarray(node_vol, dtype=np.float64)
Minv_gpu     = np.asarray(Minv, dtype=np.float64)
res_idx      = np.asarray(np.where(reservoir_mask)[0])

# Saturate the source nodes at t=0; this initial charge counts against budget.
mu_chem_gpu[res_idx] = 1.0
injected = np.float64(k_phi * node_vol[res_nodes].sum())   # m^3 released so far
print("CPU setup complete.", flush=True)
...
# ---------------------------------------------------------------------------
# Simulation loop (CPU via NumPy)
# ---------------------------------------------------------------------------

print(f"Running on CPU: {n_nodes} nodes, {n_elements} elements, {nsteps} steps...", flush=True)
...
for step in pbar:
    ...
    force_kernel_cpu(
        ...
        mu_0, N_z, lam_0, softening_factor, phi_gpu,
        f_out_gpu, n_elements, N_x, N_y, dJ_min_kernel)
    ...
    if step % frame_record == 0:
        positions_h = np.asarray(points_ref_gpu + u_gpu)
        mu_chem_h   = np.asarray(mu_chem_gpu)
        phi_h       = np.asarray(phi_gpu)
        disp_mag    = np.linalg.norm(u_gpu, axis=1)
        inj_now     = float(injected)
...
print(f"HDF5 written: {h5_filename}  ({frame_idx} frames). "
      f"Solvent injected: {float(injected)*1e9:.4f} / {V_solvent*1e9:.4f} uL", flush=True)
